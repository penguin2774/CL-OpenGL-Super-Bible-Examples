...
(defmethod glut:reshape  ((window hello-window) w h)
  (let ((ratio (float (/ w h)))
    ...
    (gl:load-identity)
    (if (<= w h)
	(gl:ortho (- n-range) n-range (/ (- n-range) ratio) (/ n-range ratio) (- n-range) n-range)
	(gl:ortho (* (- n-range) ratio) (* n-range ratio) (- n-range) n-range (- n-range) n-range))
