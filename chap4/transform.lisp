...
(defmethod render-scene ((w hello-window))
  ...
  (let ((m (m3d:rotation-matrix44 (m3d:load-identity44) (m3d:deg-to-rad (torus-rot-y w)) 0.0 1.0 0.0)))
    (watch-var (/ (scew-x w) 100))
    (watch-var (/ (scew-y w) 100))
    (setf (elt m 0) (/ (scew-x w) 100))
    (setf (elt m 1) (/ (scew-y w) 100))
    (setf (elt m 12) 0.0)
    (setf (elt m 13) 0.0) ;(scew-y w))
    (setf (elt m 14) -2.5)
    (m3d:draw-torus m))
  (glut:swap-buffers))
...
(defmethod glut:mouse ((w hello-window) button state x y)
  (when (eq button :right-button)
    (if (eq state :down)
	(setf (scew-point w) (cons x y))
	(progn
	 (setf (scew-x w) 0.0)
	 (setf (scew-y w) 0.0)
	  (setf (scew-point w) nil))))
...
(defmethod glut:motion ((w hello-window) x y)
  (when (scew-point w)
    (setf (scew-x w) (- (car (scew-point w)) x))
    (setf (scew-y w) (- (cdr (scew-point w)) y))
    (glut:post-redisplay))
  (when (drag-point w)
